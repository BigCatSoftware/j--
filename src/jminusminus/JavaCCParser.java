...
class JavaCCParser implements JavaCCParserConstants {
    ...
    final private JExpression variableInitializer(Type type) throws ParseException {
    ...
    final private JArrayInitializer arrayInitializer(Type type) throws ParseException {
        int line = 0;
    ...
    final private JExpression literal() throws ParseException {
        ...
        try {
            switch ((jj_ntk == -1) ? jj_ntk_f() : jj_ntk) {
                ...
                case HEX_LONG_LITERAL: {
                    jj_consume_token(HEX_LONG_LITERAL);
                    expr = new JLiteralHexLong(token.beginLine, token.image);
                    break;
                }
                case BINARY_LONG_LITERAL: {
                    jj_consume_token(BINARY_LONG_LITERAL);
                    expr = new JLiteralBinaryLong(token.beginLine, token.image);
                    break;
                }
                case OCTAL_LONG_LITERAL: {
                    jj_consume_token(OCTAL_LONG_LITERAL);
                    expr = new JLiteralOctalLong(token.beginLine, token.image);
                    break;
                }
                case HEX_INT_LITERAL: {
                    jj_consume_token(HEX_INT_LITERAL);
                    expr = new JLiteralHexInt(token.beginLine, token.image);
                    break;
                }
                case BINARY_INT_LITERAL: {
                    jj_consume_token(BINARY_INT_LITERAL);
                    expr = new JLiteralBinaryInt(token.beginLine, token.image);
                    break;
                }
                case OCTAL_INT_LITERAL: {
                    jj_consume_token(OCTAL_INT_LITERAL);
                    expr = new JLiteralOctalInt(token.beginLine, token.image);
                    break;
                }
                default:
                    jj_la1[53] = jj_gen;
